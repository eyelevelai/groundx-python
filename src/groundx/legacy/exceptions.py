...
from groundx.legacy.exceptions_base import (
    OpenApiException,
    ApiTypeError,
    ApiValueError,
    render_path,
...
class ApiException(OpenApiException):
    ...
    def __init__(
        ...
        status=None,
        reason=None,
        api_response: typing.Optional[
            typing.Union[ApiResponse, AsyncApiResponse]
        ] = None,
    ):
        if api_response:
...
class AnyOfValidationError(OpenApiException):
    def __init__(
        self, error_list: typing.List[typing.Union[ApiTypeError, ApiValueError]]
    ):
        ...
        if num_validation_errors == 1:
            ...
            msg = "{} validation error{} detected: \n".format(
                num_validation_errors, "s" if num_validation_errors > 1 else ""
            )
    ...
    def __init__(self, error: TypeError):
        ...
        if "__new__()" in error_str:
            ...
            number_of_parameters = (
                error_str.split("missing")[1].split("required")[0].strip()
            )
            self.msg = "Missing {} required parameter{}: {}".format(
                number_of_parameters,
                "s" if int(number_of_parameters) > 1 else "",
                missing_parameters,
            )
...
class SchemaValidationError(OpenApiException):
    def __init__(
        self, validation_errors: typing.List[typing.Union[ApiValueError, ApiTypeError]]
    ):
        ...
        self.missing_required_properties_errors: typing.List[
        ...
        if len(self.type_errors) > 0:
            for type_error in self.type_errors:
                if isinstance(type_error, MissingRequiredPropertiesError) or isinstance(
                    type_error, MissingRequiredParametersError
                ):
                    sub_msgs.append(str(type_error))
                else:
                    classes = ", ".join(
                        [cls.__name__ for cls in type_error.valid_classes]
                    )
                    msg = "Got {}({}) for required type {} at {}".format(
                        type(type_error.invalid_value).__name__,
                        type_error.invalid_value,
                        classes,
                        render_path(type_error.path_to_item),
                    )
                    sub_msgs.append(msg)
        ...
        self.msg = "{} invalid argument{}. {}".format(
            num_validation_errors, "s" if num_validation_errors > 1 else "", sub_msg
        )
        super().__init__(self.msg)
