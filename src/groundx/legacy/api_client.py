...
from groundx.legacy import rest
from groundx.legacy.api_response import ApiResponse, AsyncApiResponse
from groundx.legacy.rest import AsyncResponseWrapper, ResponseWrapper
from groundx.legacy.configuration import Configuration
from groundx.legacy.exceptions import (
    ApiTypeError,
    ApiValueError,
    MissingRequiredParametersError,
)
from groundx.legacy.request_after_hook import request_after_hook
from groundx.legacy.request_before_url_hook import request_before_url_hook
from groundx.legacy.schemas import (
    NoneClass,
    BoolClass,
    Schema,
    FileIO,
...
class OpenApiResponse(JSONDetector):
    ...
    def __get_schema_for_content_type(
        ...
        matched_media_type = OpenApiResponse.match_content_type(
            content_type=content_type, media_types=media_types
        )
    ...
    def match_content_type(
        content_type: str, media_types: typing.List[str]
    ) -> typing.Optional[str]:
        """
        ...
        for media_type in media_types:
            if media_type == "*/*" or media_type == content_type:
                ...
                if (type_ == "*" or type_ == content_type.split("/")[0]) and (
                    subtype == "*"
                    or subtype == content_type.split("/")[1].split(";")[0]
                ):
                    return media_type
    ...
    async def deserialize_async(
        ...
        if self.content is not None:
            ...
            if self._content_type_is_json(content_type):
                ...
                body_data = self.__deserialize_multipart_form_data(
            ...
            body_schema.from_openapi_data_oapg(body_data, _configuration=configuration)
    ...
    def deserialize_body(
        self, response: ResponseWrapper, content_type: str
    ) -> (any, str):
        if self._content_type_is_json(content_type):
            deserialized_body = self.__deserialize_json(response.http_response.data)
        elif content_type == "application/octet-stream":
            deserialized_body = self.__deserialize_application_octet_stream(
                response.http_response
            )
        elif content_type.startswith("multipart/form-data"):
            deserialized_body = self.__deserialize_multipart_form_data(
    ...
    def deserialize(
        ...
        try:
            deserialized_body, content_type = self.deserialize_body(
...
class ApiClient:
    ...
    def __init__(
        ...
        self.user_agent = "Konfig/1.3.29/python"
    ...
    def fields_to_dict(
        self, fields: typing.Optional[typing.Tuple[typing.Tuple[str, str], ...]]
    ):
...
class RequestBody(StyleFormSerializer, JSONDetector):
    ...
    def __serialize_multipart_form_data(
        ...
        if (
            not isinstance(in_data, frozendict.frozendict)
            and not isinstance(in_data, list)
            and not isinstance(in_data, tuple)
        ):
            raise ValueError(
                f"Unable to serialize {in_data} to multipart/form-data because it is not a dict of data or a list of data"
            )
        """
