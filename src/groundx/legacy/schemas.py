...
from groundx.legacy.configuration import (
    Configuration,
)
from groundx.legacy.exceptions import SchemaValidationError
from groundx.legacy.exceptions import render_path
from groundx.legacy.validation_metadata import ValidationMetadata
from groundx.legacy.exceptions import AnyOfValidationError
from groundx.legacy.exceptions import MissingRequiredPropertiesError
...
class Schema:
    ...
    def __type_error_message(
        ...
        msg = (
            "Invalid type. Required {0} type {1} and "
            'passed type was {2} for "{3}"'.format(
                key_or_value,
    ...
    def __new__(
        ...
        **kwargs: typing.Union[
        ...
        _validation_metadata = ValidationMetadata(
            ...
            validated_path_to_schemas=_validated_path_to_schemas,
        )
        ...
        return _new_cls._get_new_instance_without_conversion_oapg(
            _arg, _validation_metadata.path_to_item, _path_to_schemas
        )
...
class IntBase:
    ...
    def _validate_oapg(
        ...
        """
        if cls._types and int not in cls._types:
            cls._types.add(int)
            ...
            raise MissingRequiredPropertiesError(
                "{} is missing {} required propert{}{}: {}".format(
                    ...
                    (
                        " at '{}'".format(
                            ".".join(
                                [str(i) for i in validation_metadata.path_to_item[1:]]
                            )
                        )
                        if len(validation_metadata.path_to_item) > 1
                        else ""
                    ),
...
class NumberSchema(NumberBase, Schema, NumberMixin):
    """
