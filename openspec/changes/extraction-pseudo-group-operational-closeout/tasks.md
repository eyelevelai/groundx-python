...
- [ ] Confirm the first `groundx[extract]` package version that includes
      `FinalFieldPath`, `PreparedExtractionYaml`, and `prepare_extraction_yaml`
      exported from `groundx.extract`.
- [ ] If the released version is not `3.5.8`, update the harness requirement in
...
- [x] If regeneration overwrites the extract SDK work, move the behavior into
      ...
      extract extension.
- [ ] Add a packaging smoke test in the release workflow or local release
...
- Fern generation durability is satisfied for this change because
  `.fernignore` protects `src/groundx/extract` and `tests/extract` from Fern
  regeneration. No source move is currently required.
