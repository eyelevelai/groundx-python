...
- [ ] 1.1 Add root `.gitattributes` normalizing `*.py`, `*.md`, `*.json`, `*.yml`, `*.yaml`,
...
- [ ] 1.2 Add root `.editorconfig` defaulting `end_of_line = lf`, `charset = utf-8`,
...
- [ ] 2.1 Implement `scripts/check-line-endings.sh`: enumerate tracked files via `git ls-files`,
...
- [ ] 2.2 RED (inline, scratch fixture): create a temporary file outside any binary/excepted
...
- [ ] 2.3 GREEN: remove the scratch CRLF fixture, re-run `scripts/check-line-endings.sh --all`
  against the (clean, tracked-only) working tree, and confirm it exits 0 with no offenders
  reported (finalize-success polarity).
- [ ] 2.4 Confirm the scratch fixture from 2.2 was fully removed (`git status` clean) — nothing
  about this verification is committed, no new test file is added under `tests/`.

## 3. CI wiring

- [ ] 3.1 Add `.github/workflows/line-endings.yml` — new, separate workflow running
...
- [ ] 4.1 Update `.fernignore` to add all four new paths: `.gitattributes`, `.editorconfig`,
...
- [ ] 5.1 Add a row to `AGENTS.md`'s `Topic | Read when` table pointing to
...
- [ ] 6.1 Run `scripts/check-line-endings.sh --all` locally against this repo's current tracked
  tree and record the output (expect: 0 offenders, matching the confirmed baseline).
- [ ] 6.2 Run `git diff --check` after all policy-file changes and confirm it reports no
  whitespace/line-ending errors.
- [ ] 6.3 Run `poetry run mypy .` and `poetry run pytest -rP -n auto .` and confirm both pass.
