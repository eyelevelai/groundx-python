...
- [ ] 1.2 Test: end-to-end repeated-group reassembly — workflow output with list groups
...
- [ ] 1.3 Test: structural integrity still enforced — corrupt stored structures (drop a
...
- [ ] 2.2 `_normalize_persisted_custom_workflow_metadata`: STOP comparing stored `schema_hash`
...
- [ ] 2.3 Confirm `agent_chain` handling unchanged (hash never covered it; keep it that way,
      pinned by a test).
...
- [ ] 3.2 internal-arcadia-agents — NOW OWNS THE WILDCARD NORMALIZATION (fresh-scan P2):
...
- [ ] 3.3 internal-arcadia-agents follow-up (separate PR): stop silently swallowing metadata
